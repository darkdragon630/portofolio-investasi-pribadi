 ...
 * Delete Keuntungan - Soft/Hard Delete with Confirmation
 * -------------------------------------------------------
 * Menghapus record keuntungan beserta BLOB bukti_file
...
try {
    ...
    // ===================================
    $sql = "SELECT ki.id, ki.judul_keuntungan, ki.jumlah_keuntungan, 
                   ki.tanggal_keuntungan, ki.bukti_file,
    ...
    $keuntungan = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if ($confirm !== 'yes') {
        ...
        showConfirmationPage($keuntungan);
        exit;
    }
    ...
    $sqlDelete = "DELETE FROM keuntungan_investasi WHERE id = ?";
    $stmtDelete = $koneksi->prepare($sqlDelete);
    $stmtDelete->execute([$keuntungan_id]);
    ...
    $logMessage = sprintf(
        ...
        $keuntungan['id'],
        $keuntungan['judul_keuntungan'],
        number_format($keuntungan['jumlah_keuntungan'], 0, ',', '.'),
        $_SESSION['username'] ?? 'Unknown'
    );
    
    // Uncomment if log function exists
    // log_security_event("KEUNTUNGAN_DELETED", $logMessage);
    
    redirect_with_message(
        "../dashboard.php", 
        "success", 
        "✅ Keuntungan '{$keuntungan['judul_keuntungan']}' berhasil dihapus."
    );
    ...
    error_log("Delete Keuntungan Error: " . $e->getMessage());
    
    // Uncomment if log function exists
    // log_security_event("DELETE_KEUNTUNGAN_ERROR", "ID: {$keuntungan_id}, Error: " . $e->getMessage());
    
    redirect_with_message(
        "../dashboard.php", 
        "error", 
        "❌ Gagal menghapus keuntungan: " . $e->getMessage()
    );
}
...
function showConfirmationPage($data) {
    ...
    <head>
        ...
        <title>Konfirmasi Hapus Keuntungan - SAZEN v3.0</title>
            ...
            <div class="confirm-body">
                ...
                <div class="data-card">
                    <div class="data-row">
                        ...
                        <span class="data-value"><?= htmlspecialchars($data['judul_keuntungan']) ?></span>
                    </div>
                    <div class="data-row">
                        <span class="data-label">
                    ...
                    <div class="data-row">
                        ...
                        <span class="data-value">Rp <?= number_format($data['jumlah_keuntungan'], 0, ',', '.') ?></span>
                    </div>
                    <div class="data-row">
                        <span class="data-label">
                        ...
                        <span class="data-value"><?= date('d M Y', strtotime($data['tanggal_keuntungan'])) ?></span>
                    </div>
                    <?php if ($hasFile): ?>
                    <div class="data-row">
                        <span class="data-label">
