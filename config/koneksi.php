 ...
 * Updated: JSON-based file storage + FIXED parse_currency_fixed
 */
...
define('DB_NAME', 'dbVeB9yLyAZeyStPbf36Ju3y');
define('DB_USER', 'f3544ccb7b708000db809fc4fe29');
define('DB_PASS', '068ff354-4ccc-711a-8000-62c09ad42c24');
...
/**
 * Parse currency input correctly - SUPER FIXED VERSION v3.0
 * Handles formats: 1500000, 1.500.000, 1,500,000, 0.82, 0,82, 4, .82, ,82
 * 
 * FIXED ISSUES:
 ...
 * - ✅ FIX CRITICAL: 0.82 tidak salah dianggap sebagai Rp 82
 * - ✅ FIX: Debug log yang aman tanpa $_POST dependency
 */
if (!function_exists('parse_currency_fixed')) {
    function parse_currency_fixed($value) {
        // Simpan nilai original untuk debug
        ...
        // If only dots (could be thousand separator or decimal)
            ...
            if ($dotCount > 1) {
                ...
                if (count($parts) === 2 && isset($parts[0]) && isset($parts[1])) {
                    ...
                    // ⭐ CRITICAL FIX: Check if integer part is 0 or empty
                    if ($integerPart === '0' || $integerPart === '') {
                        // Format: 0.82 or .82 = desimal, BUKAN ribuan!
                    ...
                    // Check decimal length
                        ...
                        // Ambiguous: could be 1.500 (thousand)
        ...
        // Debug log untuk troubleshooting (FIXED - tidak depend pada $_POST)
        error_log(sprintf(
            "[parse_currency_fixed] INPUT='%s' → PROCESSED='%s' → OUTPUT=%.4f",
            $original_value,
            $value,
            $result
        ));
 ...
 */
if (!function_exists('handle_file_upload_to_db')) {
 ...
 */
if (!function_exists('parse_bukti_file')) {
 ...
 */
if (!function_exists('display_file_from_db')) {
